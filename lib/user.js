...
module.exports = Class.create({
	...
	api_callback: async function (args, callback) {
		...
		try {
			...
			const tokenRequestPayload = Tools.mergeHashes({
			...
			if(oauth.token_url.startsWith('https://login.microsoftonline.com/')) {
				tokenResponse = await self.postJsonAsync(oauth.token_url, {
					data: Buffer.from(new URLSearchParams(tokenRequestPayload).toString()),
